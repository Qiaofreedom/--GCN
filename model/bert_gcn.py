...
class BertGCN(nn.Module):
    def __init__(self, edges, features, config, args): # edges和features是分别代表边和特征
        ...
        self.edges = edges  # Aij, 相关矩阵里面的值 维度是c*c
        self.device = args.device
        self.dropout = nn.Dropout(config['dropout_prob'])
        
        self.bert = AutoModel.from_pretrained(args.pretrained)  # 加载一个预训练的BERT模型。模型的具体类型由args.pretrained指定，这通常是一个模型的名称或路径。
        self.gc1 = GCNLayer(features.size(1), self.bert.config.hidden_size)  # 输入特征维度为 features的第二维大小c，输出特征维度为 BERT模型隐藏层的大小d'。
        self.classifier = nn.Linear(self.bert.config.hidden_size, self.label_features.size(0))  # 输入维度为BERT输出的隐藏状态维度d'，输出维度为 标签的数量c。
    ...
    def forward(self, input_ids, attention_mask):
        ...
        bert_output = self.dropout(bert_output)  # 文本+Bert算法. 文档嵌入T′经过一个BERT模型和线性层变换，最终的维度也被设定为 d′.
        ...
        output = torch.zeros((bert_output.size(0), label_embed.size(0)), device=self.device)  # 初始化一个输出张量，大小为d' 乘以 标签数量c，准备存储 每个文本样本 对应 每个标签的预测结果。
